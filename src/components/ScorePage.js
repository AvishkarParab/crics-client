...
//******************************* */
const [toss,setToss] = useState({won:"",elect:""});
const teamapage = async()=>{
    try{
      ...
      const data = await res.json();
      setToss({...toss,won:data.won,elect:data.elect});
...
const bowlerpage = async ()=>{
  try{
    ...
    const res = await fetch('/choosebowl',{
      method:"POST",
      headers:{
          "Content-Type":"application/json"
      },
...
console.log("Overs "+score.totover);
  const st =(e)=>{
    if(e.target.style.backgroundColor === "yellow"){
      setScore({...score,[e.target.name]:Number(-1)});
      e.target.style.backgroundColor="transparent";
      e.target.style.color="rgb(60, 255, 0)";
    }else{
        ...
        if(score.one === 1){
          if(score.nb ==="true"){
            ...
            if((bowler.bwballs+1)%6 ===0){
              ...
              var inni = bowlup(1)
              if(inni)
              navigate('/aselectbat',{state:{target:score.totscore+1}});
              else
              navigate('/selectbowl');
            }
            document.getElementById('one').style.backgroundColor="transparent";
        ...
        if(score.two === 2){
          if(score.nb ==="true"){
            ...
            if((bowler.bwballs+1)%6 ===0){
              ...
              var inni = bowlup(2)
              if(inni)
              navigate('/aselectbat',{state:{target:score.totscore+2}});
              else
              navigate('/selectbowl');
            }
            document.getElementById('two').style.backgroundColor="transparent";
        ...
        if(score.three === 3){
          if(score.nb ==="true"){
            ...
            if((bowler.bwballs+1)%6 ===0){
              ...
              var inni = bowlup(3)
              if(inni)
              navigate('/aselectbat',{state:{target:score.totscore+3}});
              else
              navigate('/selectbowl');
            }
            document.getElementById('three').style.backgroundColor="transparent";
        ...
        if(score.four === 4){
          if(score.nb ==="true"){
            ...
            if((bowler.bwballs+1)%6 ===0){
              ...
              var inni = bowlup(4)
              if(inni)
              navigate('/aselectbat',{state:{target:score.totscore+4}});
              else
              navigate('/selectbowl');
            }
            document.getElementById('four').style.backgroundColor="transparent";
...
if(score.five === 5){
  if(score.nb ==="true"){
    ...
    if((bowler.bwballs+1)%6 ===0){
      ...
      var inni = bowlup(5)
      if(inni)
      navigate('/aselectbat',{state:{target:score.totscore+5}});
      else
      navigate('/selectbowl');
    }
    document.getElementById('five').style.backgroundColor="transparent";
...
if(score.six === 6){
  if(score.nb ==="true"){
    ...
    if((bowler.bwballs+1)%6 ===0){
      ...
      var inni = bowlup(6)
      if(inni)
      navigate('/aselectbat',{state:{target:score.totscore+6}});
      else
      navigate('/selectbowl');
    }
    document.getElementById('six').style.backgroundColor="transparent";
...
if(score.zero === 0){
  if(score.nb ==="true"){
    ...
    if((bowler.bwballs+1)%6 ===0){
      ...
      var inni = bowlup(0)
      if(inni)
      navigate('/aselectbat',{state:{target:score.totscore+0}});
      else
      navigate('/selectbowl');
    }
    document.getElementById('zero').style.backgroundColor="transparent";
 ...
 const out = async() =>{
  console.log(bowler.bwname , bowler.bwovers , bowler.bwmaidens , bowler.bwruns, bowler.bwwickets+1 , bowler.bwer, (bowler.bwballs+1)%6);
    ...
    try{
      const res = await fetch('/updatebowler',{
        method:"PUT",
        headers:{
            "Content-Type":"application/json"
        },
    ...
    try{
      ...
      setScore({...score,totscore:0,totover:0,totball:0,totwicket:0});
      const data = await res.json();
      if(!res.status=== 200){
        const error = new Error(res.error);
        throw error;
...
if(bats.b1strike){
  ...
  batup(bats.b1name,bats.b1runs,bats.b1balls,bats.b1fours,bats.b1sixes,bats.b1sr)
  batup(bats.b2name,bats.b2runs,bats.b2balls,bats.b2fours,bats.b2sixes,bats.b2sr)
  console.log(bowler.bwballs);
  navigate('/selectbat');
  setOpen(location.state.bat);
...
if((bowler.bwballs+1)%6 ===0){
  ...
  async function batup(name,runs,balls,fours,sixes,sr){
    try{
      const res = await fetch('/updatebatsman',{
        method:"PUT",
        headers:{
            "Content-Type":"application/json"
        },
  ...
  return (
     <>
        ...
        <div className='container-fluid scorebox d-flex justify-content-center'>
          <div className='mt-2 scorecontainer'>
          <div className='text-center d-flex justify-content-center align-items-center' style={{backgroundColor:"#f5f505",color:"black",borderRadius:"7px",padding:"5px"}}>
            <h5 className=''>{team.aname} vs {team.bname}</h5>
          </div>
          <div className='text-center mb-1 bg-info d-flex justify-content-center align-items-center' style={{backgroundColor:"#f5f505",color:"black",borderRadius:"5px",paddingTop:"3px"}}>
            <h6 className=''>{toss.won} won the toss and elected to {toss.elect}</h6>
          </div>
          <div className='viewscore '>
            <div className=' row mx-0'>
                ...
                <div className=' col-lg-9 col-md-9 col-8 text-center'>
                  ...
                  <div className='col-12 text-start' style={{color:"yellow"}}>
                  <h5 className=''> {score.timeline}</h5>
                  </div>
                </div>
            </div>
            ...
            <div className=' row mx-0 mb-2'>
              <div className='player1name col-4 text-center'>
                  <div className='bat1name text-start' style={{color:"yellow"}}>
                    <h6 className='' id='p1' name='p1'>{bats.b1name?bats.b1name:"Player1"}</h6>
                  </div>
              </div>
          ...
          <div className='scorebtns mt-3'>
            <div className='text-light text-center' style={{backgroundColor:"green",height:"25px",padding:"3px"}}>
              <h6>Update Score</h6>
            </div>

            <div className='row mx-0 text-center mt-2 mb-2'>
              <div className='col-2'>
