...
const [player,setPlayer] = useState({
  ...
  const select = async()=>{
    try{
      ...
      const res = await fetch('/afterselectbat',{
        method:"GET",
        headers:{
          "Content-Type": "application/json",
        }
...
const proceed=(e)=>{
  e.preventDefault();
  if(open){
  navigate('/aselectbowl',{state:{target:location.state.target,open,bat}});
}
}
        ...
        <div className='headerbox d-flex align-items-center justify-content-center'><h2 className=''>SELECT BATSMAN</h2></div>
          ...
          <br />
            ...
            <div className='mt-2 text-center'>
              <button className='btn btn-outline-light' type='submit' onClick={check}>Select</button>
              <button className='btn btn-outline-light mx-5' type='submit' onClick={proceed}>Proceed</button>
            </div>
           </form>
          </div>
    </>
