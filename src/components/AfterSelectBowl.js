...
const [player,setPlayer] = useState({
  ...
  const select = async()=>{
    try{
      ...
      const res = await fetch('/afterselectbowl',{
        method:"GET",
        headers:{
          "Content-Type": "application/json",
        }
...
const proceed=(e)=>{
  e.preventDefault();
  navigate('/afterscorepage',{state:{target:location.state.target,bat:location.state.bat,open:location.state.open,bowl:bowl}});
}
  
  return (
    <>
        <div className='headerbox d-flex align-items-center justify-content-center'><h2 className=''>SELECT BOWLER</h2></div>
          <div className=' bg-danger titlebox d-flex align-items-center justify-content-center'><h2 className=''>TEAM {name}</h2></div>
          <div className='mt-2 mb-2 selbowlbox container'>
          ...
          <br />
            ...
            <div className='mt-2 text-center'>
              <button className='btn btn-outline-warning' type='submit' onClick={check}>Select</button>
              <button className='btn btn-outline-warning mx-5' type='submit' onClick={proceed}>Proceed</button>
            </div>
           </form>
          </div>
    </>
...
export default AfterSelectBowl;
